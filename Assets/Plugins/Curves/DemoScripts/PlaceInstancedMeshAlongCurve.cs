...
public class PlaceInstancedMeshAlongCurve : MonoBehaviour
{
    ...
    void BuildMesh(Curve c){
        ...
        for(int i = 0; i < numberOfObjects; i++){
            ...
            combine[i].transform = transform.worldToLocalMatrix * Matrix4x4.TRS( p + f * offset.z + u * offset.y + r * offset.z, Quaternion.LookRotation( f,u) *Quaternion.Euler( eulerRotation)  , Vector3.one * s * scaleMultiplier);
            combine[i].mesh = mesh;
