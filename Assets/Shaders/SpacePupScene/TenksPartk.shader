Shader "Scenes/SpacePup/TenkPArKs" {
  ...
  Properties {
    ...
    SubShader {
       ...
       // if( length(v.tan)> .5){ discard;}
        ...
        col = 2*tCol* tex2D(_ColorMap, float2(v.debug.y * .2 + .6 - d.x * .4 * d.a ,0));//-rM;//v.nor * .5 + .5;// tCol;//*tCol * tex2D(_ColorMap,float2(rM*.1+.7 - v.debug.y * .1 ,.5 )).rgb;// *(1-rM);//hsv(rM*rM*rM * 2.1,.5,rM);// + normalize(refl) * .5+.5;
        //col = v.tan * .5 + .5;
        //col *= (d.xyz);
