Shader "Scenes/CaveScene/Katenarie"
{
    Properties {
...
float3 refl = reflect( normalize(_WorldSpaceCameraPos-v.worldPos) , v.nor);
        float reflM = dot( refl , _WorldSpaceLightPos0 );
                ...
                float4 aCol = tex2D(_AudioMap,  float2(v.uv.x  * .3  + sin(v.kelpID) * .1 + tCol.x * .001 , 0) );
               ...
               // if( ( lookupVal + 1.3) - 1.2*length( tCol ) < .5 ){ discard;}
                fixed4 col = aCol * float4( cCol , 1 )  * 2* tex2D(_ColorMap , float2( val * .2 + sin( v.kelpID) * .2 +  _HueStart, 0) );// * saturate(20*-val);//* 20-10;//*tCol* lookupVal*4;//* 10 - 1;
