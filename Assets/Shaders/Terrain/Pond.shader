  ...
  StructuredBuffer<int> _TriBuffer;
    ...
    float _MapHeight;
            ...
            varyings vert(input v)
            {
                ...
                float3 nor = normalize(mul( unity_ObjectToWorld , float4(v.normal,0)).xyz);
                float3 worldPos = mul( unity_ObjectToWorld , v.vertex ).xyz;
                //worldPos.y = _ExtraHeight;


                float3 pos =worldPos;// displace(worldPos,nor);
...
//
               ...
               // col = tex2Dproj(_BackgroundTexture,ComputeGrabScreenPos( mul(UNITY_MATRIX_VP, float4(v.world + v.eye*.9,1)))).rgb;;// + saturate(aroundPerson * (lookup));
                ...
                // make a fade when its deeper
                col = lerp( col , col *float3(0,.3,.5) , saturate(foamLine* foamLine * .01));// (1+(.2*dif*dif));
