       ...
       float _RayLocationOsscilationSize;
        ...
        float3 getPos( float x , float y , float3 left , float3 up , float3 forward , float3 pos){
            ...
            v2f vert ( uint vid : SV_VertexID )
            {
                ...
                float3 startPos = getPos( x , y , left, up , forward , pos );//2*(x - .5) * _CameraSize.x * left + 2*(y-.5) * _CameraSize.y * up + forward * _CameraNear + pos;
                ...
                float3 p1 = startPos - l;
                float3 p2 = startPos + l;
                float3 p3 = endPos   - l;
                float3 p4 = endPos   + l;
