Shader "Scenes/HydraScene/DiscardHydraBody"
{
    Properties {
      ...
      _DiscardDistance("_DiscardDistance", float ) =1
            ...
            fixed4 frag (v2f v) : SV_Target
            {
              ...
              //  col += col *pow(rM,5)*20;
               ...
               // float3 audio = SampleAudio(v.uv.x * .1 + p.x * .03 );
                ...
                float playerD = length(v.world.xz - _PlayerPosition.xz); 

                if( playerD < _DiscardDistance ){
                    discard;
                }
               // col.xyz *= (audio *audio*10 + 1);
                col.xyz *= reflectionColor * 4;

                    float4 audio = SampleAudio(length(reflectionColor.xyz) * .05 + playerD * .1) * 2;
                col  *= .3+audio.xyz;
                col = GetGlobalColor( m * _ColorSize  + _ColorBase )* (.3+audio.xyz);
