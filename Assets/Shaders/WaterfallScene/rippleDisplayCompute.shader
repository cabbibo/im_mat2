...
Blend SrcAlpha OneMinusSrcAlpha
        ...
        Pass
        {
            ...
            v2f vert ( uint vid : SV_VertexID )
            {
                ...
                float3 fPos = v.pos + float3(0,saturate(col.x * .1),0);

                o.pos = mul (UNITY_MATRIX_VP, float4(fPos,1.0f));

                o.displacement = col.x;
      ...
      samplerCUBE _CubeMap;   
         ...
         float3 delta = float3(.004 , 0 , 0 );
                ...
                float3 pU  =  v.worldPos + float3(0,colU*.05,0) + delta.xyy;
                float3 pD  =  v.worldPos + float3(0,colD*.05,0) - delta.xyy;
                float3 pL  =  v.worldPos + float3(0,colL*.05,0) + delta.yyx;
                float3 pR  =  v.worldPos + float3(0,colR*.05,0) - delta.yyx;
                ...
                float4 info = tex2D(_MainTex, float4(v.uv,0,0));
                float3 dif = terrainPos - v.worldPos;
                float dist = 2-((abs(dif.y) * 100 + .5*noise( v.worldPos * 20 + float3(0,0,_Time.y)) + .3 * noise( v.worldPos * 10 + float3(0,0,_Time.y))));
                float3 cCol = tex2D(_ColorMap,float2(info.x * 1,0));

                //if( dist > 1 - noise( v.worldPos * 20 + col.x) ){ discard; }
                col.a = saturate(-dist * .1);
                
                 col.xyz =  tCol * cCol * 2 * info.x;//saturate(10*dist);// + 1-10*saturate(dot( normalize(v.eye) ,  fNor ));//1/dist;// * 1.1;
                return col;
            }
            ENDCG
        }
