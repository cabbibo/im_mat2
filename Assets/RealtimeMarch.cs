...
public class RealtimeMarch : Simulation
{
  ...
  public void DoMeshTile(int i ){
    ...
    int z = (i /(size*size));
      ...
      Vector3 dim = ((Form3D)form).dimensions;

      Vector3 extra = new Vector3(tileSize/dim.x,tileSize/dim.y,tileSize/dim.z);
      ((Form3D)form).extents = new Vector3(tileSize,tileSize,tileSize) + 1.04f*extra;
